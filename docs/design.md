...
2. 各画像から `created_at` をファイル名ベースで抽出する
3. 各画像の必要領域を切り出す
4. 攻撃/防衛アイコンを判定する
...
- 画像ファイル名に含まれる時刻を `created_at` として扱う
...
このため、`created_at` はファイル名から日付部分と時刻部分を抜き出して生成する。
...
対象領域を固定座標で切り出し、画像から形状特徴を抽出して判定する。

### 推奨する実装方式

1. 左右それぞれのユーザー領域から、役割アイコンの表示位置を固定座標で切り出す
2. 切り出した画像から前景の重み、細長さ、斜め方向の相関を計算する
3. 攻撃アイコンと防衛アイコンの特徴スコアを計算する
4. スコアが低い方を判定結果として採用する
...
- 文字コードの BOM は付与しない
- 空欄は空文字として出力する
- ヘッダー行は画面上の「ヘッダーを付与」チェックボックスで切り替える
- 「ヘッダーを付与」はデフォルトではオフとし、選択状態はブラウザに保存する
...
- 画像名から `created_at` を抽出できない場合は中断する
