...
最終的な出力は固定フォーマットの CSV 文字列とし、ファイル保存ではなく画面上に表示してコピーできるようにする。
...
- 抽出結果を固定の CSV フォーマットで生成できること
- CSV はファイルではなく画面上のテキストボックスに表示すること
...
- 画像アップロード、OCR、アイコン判定、CSV 生成までをブラウザ内で処理する
...
内部処理と CSV 出力では、ソートや比較がしやすい `YYYYMMDDHHMMSS` 形式を採用する。
...
CSV は、後続の絞り込み、集計、比較をしやすくするため、1 戦闘を 1 行で表現する。

また、画像上の左右位置をそのまま出力するのではなく、意味上の役割で列を正規化する。

- 左側の列群には常に攻撃側の情報を出力する
- 右側の列群には常に防衛側の情報を出力する
- 画像上で攻撃側と防衛側の表示位置が入れ替わっていても、CSV 出力時に並び替える
